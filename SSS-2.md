...
Seizer wallet
    |
    | calls sss-core::seize(from, treasury, amount)
    |
    v
sss-core program
    |
    | validates: seizer role, not paused, permanent delegate enabled
    | builds transfer_checked instruction
    | signs with Config PDA (permanent delegate authority)
    |
    v
Token-2022 transfer_checked
    |
    | Config PDA is permanent delegate -> authorized
    | invokes transfer hook (if enabled)
    |
    v
sss-transfer-hook::transfer_hook
    |
    | owner == Config PDA -> ALLOW (bypasses blacklist)
    |
    v
Transfer completes: tokens move from target to treasury
    |
    v
